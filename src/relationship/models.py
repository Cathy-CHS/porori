...
class KREModel(nn.Module):
    ...
    def __init__(self, args, n_training_steps=None, n_warmup_steps=None):
        ...
        self.classifier = nn.Linear(
            self.bert.config.hidden_size * self.scale, args.n_class
        )
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        if self.args.mode == "ALLCC":
            ...
            for h_start, h_end, t_start, t_end in zip(
                h_start_list, h_end_list, t_start_list, t_end_list
            ):
            ...
            for i, idx_list in enumerate(special_token_idx):
                if i == 0:
                    ...
                    concat_state = torch.cat(
                        [
                            concat_state,
                            last_hidden_state[i, idx_list].flatten().unsqueeze(0),
                        ],
                        dim=0,
                    )
